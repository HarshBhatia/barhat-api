...
export default function Controller<T extends mongoose.Document>(
  ...
  return {
    get: async (req: Request, res: Response) => {
      ...
      // @ts-ignore
      const document: T | null = await Model.findOne({ id });

      if (!document) {
        return res.status(404).json({
          message: `A ${objectTitle} does not exist with the id provided`,
        });
      }
    ...
    update: async (req: Request, res: Response) => {
      ...
      // @ts-ignore
      const result = await Model.updateOne({ id }, { $set: { ...updates } });
